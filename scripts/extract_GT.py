...
if __name__ == '__main__':
    ...
    GT = pd.read_csv('TownCentre-groundtruth_modified.top', header=None)
    ...
    #resulting resolution: (960, 540)

    f_to_Extract = []

    for value in GT[1]:
        if value not in f_to_Extract:
            f_to_Extract.append(value)

    #repeat train_size times
    for frame_number in range(train_size):

        if frame_number not in f_to_Extract:
            continue
